...
Verified against src/ on main (v0.4.360, HEAD 733760b) and re-checked against
origin/orch/integration (v0.4.361). Every flag/command pairing in §2 was dumped
programmatically from src/schema.js (COMMANDS, FLAGS, SUBCOMMAND_FLAGS,
GLOBAL_FLAGS) rather than transcribed; the config leaf list in §6 was dumped from
a live `load()` on an empty temp dir.

OPEN-owner-decision callouts left: 0. All six are decided and written as prose:

  1. §3.3  `agent add --build` keeps `noMerge` — the scaffolded adapter stays on
           its local branch for a human to land. Consequence recorded on the
           `--pr` row in §2.4: `--pr` flips BOTH `merge: "pr"` (cli.js:1744) and
           `noMerge: false` (cli.js:1779), and it is the second that makes a
           build open a PR. P12 must route `landing: pr` through the build
           path's `noMerge` or that migration row's replacement is inert.
  2. §3.6  a draft PR is refused under ready/merged — one readiness predicate.
           Already true in source: readiness.js:63-64 / landing.js:136-137 ->
           REMOTE_PR_CLOSED, whose only remedy is `ask` (failure.js:133), so the
           real path is ask -> exit 4, not a bare exit. No marker.
  3. §3.7  bare `continue` inherits the run's recorded goal; explicit --until
           overrides. Inheritance is ALREADY LIVE (cli.js:2992-3003); only the
           explicit override is blocked (schema.js:292-294), so the not-landed
           marker was narrowed to the override alone.
  4. §4.3  `--allow-large-scope` survives unchanged, advisory only.
  5. §6.1  `landing: pr` survives; three legal values.
  6. §6.9  `automation.remedies` default is `null` = the failure-table order.
           Omitting `ask` is legal and NOT a floor: chooseRemedy filters
           `allowed` to the operator list, `if (!allowed.length) return
           terminalDecision(row)` -> STOPPED_AT_CAP, i.e. exit 2 instead of 4.
           Operator order wins over row order; integration-repair/wait are never
           orderable or disableable (failure.js:177-195).

TO FIX ELSEWHERE (not edited from here): design §15 and orch.example.yml both
print the literal `[rebase, rotate, reauthor, ask]` as the default of
`automation.remedies`. src/config.js:60 is `null`. Those two sources are wrong
and need their own change. Not merely a different spelling of the same thing:
worked through chooseRemedy, DIFF_EMPTY's row order is `reauthor, rotate, ask`,
and the literal list re-slots it to `rotate, reauthor, ask` — so copying that
"default" into a real config changes behaviour on an empty diff. §6.9 states
that example.

Recon's v05.openDecisions is a 12-item prose blob, not the 5 the brief mentions.
The remaining six (exit-4 release ordering; partially-migrated 0.4.x config
behaviour; `merge:`+`landing:` set together; poller ownership;
harness/orch-loop.sh) are migration-guide material and were deliberately not
raised here. The sixth — rotateModels vs pinned pools — IS answered in §6.9 now,
from source rather than by decision: the ladder resumes after the pinned model.

WHERE RECON AND THE SOURCE DISAGREE

1. "--merge is honoured only by `orch pr`" (framed as an inert-flag case).
   FALSE on the current checkout. COMMANDS declares `merge` only on `pr`, so
   passing it anywhere else is exit 64, not a silent no-op. Recorded in §4.5 as
   an explicitly-not-inert case.

2. "orch pr --merge never calls prChecksGreen/statusCheckRollup (github.js:
   163-247, issue #508)". The function recon is describing, `runPr`
   (src/github.js:340), is DEAD CODE on this checkout — `grep -rn runPr src/`
   returns only its own definition. The live `pr` path is the unified
   task/review/issue/pr handler at src/cli.js:2076, where
   `flags.merge` is already just an alias: `const until = flags.until ||
   (command === "pr" && flags.merge ? "merged" : "once")`. Worth filing as
   dead-code removal for P12.

3. Exit 4 / openDecision #7 ("which release contains exit 4?"). Answerable from
   source, not open: `EXIT_FOR_STATE.WAIT_TIMEOUT = 4` (run-controller.js:11) and
   a real producer exists at src/remedies/ask.js:91. P7 has landed. Documented as
   current, scoped to `--until ready|merged`.

4. `COMMANDS.agent.flags` is the UNION of `agent add` and `agent build` flag sets
   by design (schema.js says so verbatim). Any matrix generated by iterating
   COMMANDS — including recon's — will claim `orch agent add x --pr` is flatly
   legal. validateAgentArgs rejects it CONDITIONALLY: `if (known || !flags.build)`
   (schema.js:493). So `orch agent add <registered> --pr`, and `orch agent add
   <anything> --pr` without `--build`, are exit 64 — but `orch agent add
   <unregistered> --build --pr` passes validation and reaches buildAgent, where
   `--pr` really does flip `merge: "pr"` (cli.js:1744) / `noMerge: false`
   (cli.js:1779).
   Verified live: exit 1 (`no CLI named "mistral" found on PATH`), not 64. §2.3
   documents the union as an implementation detail and states both directions.

5. Design-doc vs landed-schema divergences that P12 does not explicitly resolve.
   Documented per the landed schema (source wins); listed here so a later pass
   can decide:
   - docs/cli-v2-design.md:122 puts `author` in RUN_FLAGS and gives `pr` the full
     RUN_FLAGS set, and docs/cli-v2-proposal.md:292 lists `--author` for `pr`.
     The landed schema deliberately EXCLUDES author/authors from `pr` with an
     explicit comment ("pr audits an existing PR/branch, it never assigns an
     author"). Documented as not-accepted.
   - The design sketch gives `--json` to `init`, `upgrade`, `release` and
     `agent add`; the landed schema does not. Documented per source.
   - Larger half of that same divergence: design §3 gives `agent add` the whole
     RUN_FLAGS set — `--until`, `--max-attempts`, `--detach`, `--no-tidy`,
     `--json` — "valid only with --build". The landed `SUBCOMMAND_FLAGS["agent
     build"]` has none of those. Documented per source, and now resolved in the
     owner's favour: a `--build` cycle never lands, so `--until`/`--no-tidy`
     have nothing to describe. Cited in §3.3.
   - `--cheap` is legal on `review` today and absent from `pr` in both the
     landed schema and design §3, so the review→pr fold silently drops it with
     no migration target. Flagged in §3.15 with a workaround; needs an owner
     call on whether `pr` should gain `--cheap`.
   - The design sketch marks `config` as `mutates: false` and `completion` as
     taking no flags; landed code has `config` mutating (the wizard) and
     `completion` taking `--dry` for `install`. Documented per source, with the
     P12 `config` change marked not-yet-landed.
   - `--allow-large-scope` is absent from the entire v2 design set. The owner
     kept it unchanged and advisory; §4.3 says so and notes the design-set gap
     so a design-derived regeneration does not drop it.

6. `orch release` reads `flags["config-file"]` (src/cli.js, the release handler)
   but `COMMANDS.release.flags` is `["dry"]` only — so validate() rejects the flag
   and that read can only ever see `undefined`. Harmless dead read; not documented
   in the body, since documenting it would imply the flag works.

7. Stale source comment worth a follow-up: src/cli.js:2362-2363 says "3 = blocked
   by a policy/capacity limit, distinct from 2 (the cycle ran and did not agree)
   — a caller can retry a 3, not a 2." The v0.5 contract is the other way round:
   2 is the resumable state (`orch continue` grants a fresh attempt budget) and 3
   is the one needing a human. The CODE is right (raiseExitCode ranks 2 above 3);
   only the comment's advice is inverted.

8. Recon's config count of 42 reconciles: 41 value-bearing leaves from a live
   `load()`, plus `reviseCap`, which is in CONFIG_KEYS but not DEFAULTS because
   pickRoundCap resolves it per source. Stated explicitly in §6 rather than
   asserted.

9. §1.3's parser rules were re-verified against `parse()` (src/cli.js:482) after
   a first draft written from design prose. Two corrections landed, both in the
   direction of the code being STRICTER than the design:
   - Unknown flag IS exit 64 today — `parse()` catches parseArgs's throw and
     re-raises via `usageError()`. Recon's exit table calls this "1 (parseArgs
     throw)" today; that is stale.
   - Repeated value flags: the design says "last one wins for scalars; error only
     for contradictory `--until`". The code rejects ANY repeated non-boolean flag
     via the `tokens: true` scan, regardless of whether the values agree.
     Documented per source.

10. `--max-attempts` is documented here as a v0.5 run-command flag (int ≥ 0,
   default `automation.maxAttempts`) on the strength of docs/cli-v2-design.md:95
   and :122 and docs/cli-v2-proposal.md:230,263,290-293. It does NOT exist in
   src/schema.js today; marked not-yet-landed everywhere it appears.

11. BRANCH CHECK — main vs origin/orch/integration (read-only; no checkout).
   `git diff main origin/orch/integration -- src/` touches exactly three files:
   cli.js, config.js, remedies.js. src/schema.js is BYTE-IDENTICAL, which is why
   every schema-dependent "Not yet landed (P12, #528)" marker in this document
   is still accurate on either branch — removed commands, --max-attempts, the
   config wizard, the --until default flip, the agent add/build fold.
   One draft claim was falsified by integration and has been corrected:
   - `automation.rotateModels` was documented as "validated but inert". FALSE on
     integration: `nextModelRole` (remedies.js:175-188) consumes it inside
     rotateRemedy, config.js:61's own comment changed from "remain inert" to
     "consumed by the rotate remedy", and validation grew adapter-name and
     duplicate-model checks (config.js:227-236). Landed as d8c0c7e + five
     model-scoped-exclusion follow-ups, merged 4850610 (Closes #567). §6.9 now
     documents the live behaviour and dates it to integration.
   `env.passthrough` was re-checked on integration and is genuinely still inert
   (config.js:68; no consumer in cli-adapter.js) — left as written.
   Two other source facts corrected while checking: `if (flags.pr) cfg = { ...cfg,
   merge: "pr" }` is cli.js:1744 on main, not 1794 (two citations fixed); and a
   `noMerge` cycle returns `approved`, never `merge-deferred` (engine.js:466), so
   §3.3's exit-code paragraph no longer claims a build can be merge-deferred.
