...
Verified against TWO refs, because they differ. Baseline: `main` at 733760b
(package version 0.4.360). Forward state: `origin/orch/integration` at 39177e8
(v0.4.361), which is AHEAD of main. Only four paths differ between them —
`src/cli.js`, `src/config.js`, `src/remedies.js` and `docs/` — so every claim
whose subject lives in any other file is identical on both refs, and the
"Not yet landed" markers below were audited on that basis.

WHERE origin/orch/integration CONTRADICTED THE EARLIER DRAFT (all corrected):

A. **`automation.rotateModels` is no longer inert.** The earlier draft said the
   key was "validated, consumed by nothing yet" (§5.1's YAML comment and the
   §5.1 `env.passthrough` note) and §5.2's OPEN callout rested on the premise
   that "there is no model escalation today at all". False on integration.
   `d8c0c7e` ("fix(rotate): use configured model ladders") plus `9ccbe33`,
   `cb3c5b7`, `27d99e0`, `9400729` and `9cbc192` wire it into `rotateRemedy`
   (`src/remedies.js`: `nextModelRole`, `markExhausted`, `exclusionKey`) and
   tighten validation (`src/config.js:225-236` now rejects unknown adapter names
   and duplicate models). Rewritten as current behaviour with the four adjacent
   facts the owner's decision did not mention: the ladder is a FALLBACK guarded
   by `if (!nextAuthor || !nextReviewers.length)`, a pinned model absent from its
   ladder disables the ladder for that seat, the adapter must declare
   `capabilities.model`, and an exhausted ladder re-excludes the adapter by name.

B. **§4.3's rung-2 prose was made wrong by A.** It said an exhausted seat with no
   diverse pool member "stops the run at cap with exit 2 … With the default
   two-agent pool, that is the ordinary outcome." True only with no ladder
   configured. Rewritten as a two-pass remedy.

C. **`env.passthrough` is still genuinely inert** (`src/config.js:68`; read only
   by `validate` at `:248-251`). The draft was right about that half and wrong
   only to lump `rotateModels` in with it. The two are now documented separately.

D. **`src/cli.js:1816`'s `noMerge: !flags.pr` contradicted the §3.6 migration
   row**, which read `orch agent add mynewagent --build  # landing mode comes
   from .orch/orch.yml`. A build never lands under any `landing:` value, so that
   comment was false. Corrected, and the `landing: pr` relationship restated: on
   the build path `--pr` bought a review SURFACE (a PR instead of a bare branch),
   never a merge.

Where recon.json and the source disagreed, the source won; those disagreements
are:

1. **P5–P11 have landed further than recon's "designed, not implemented" framing
   implies.** `src/run-controller.js`, `src/failure.js`, `src/remedies.js`,
   `src/remedies/ask.js`, `src/remedies/reauthor.js`, `src/readiness.js`,
   `src/integration-repair.js` and `src/run-record.js` all exist and are wired:
   `cli.js` imports `runUntil` and calls it on the task/issue/review/pr paths
   (cli.js:15, 2545) and on the continue path (cli.js:3093). So `--until
   ready|merged`, the remedy ladder, the durable run record and exit codes 3/4
   are LIVE, not future. I marked as "not yet landed" only the P12 cutover items
   (default flip, command/flag/key removals, banner, wizard, #532 pool
   semantics).

2. **Exit code 4 is already implemented**, contrary to recon's open decision 7
   ("which release actually contains exit code 4?"). `EXIT_FOR_STATE` in
   src/run-controller.js:11 maps `WAIT_TIMEOUT: 4`, and `EXIT_CODE_PRIORITY` in
   src/cli.js:1387 ranks all five codes. I therefore documented the five-code
   contract as current and did not raise an OPEN for it.

3. **The closed config schema is already live**, contrary to recon breaking
   change [23] which dates it to v0.5.0. `src/config.js:397-398` —
   `const problems = problemList(...); if (problems.length) throw new Error(...)`
   — makes an unknown key fatal at `load()`, on every command, today. Documented
   as current.

4. **recon open decision 4 (`automation.remedies` has two contradictory
   defaults) is not a live contradiction, and the brief's version of it is half
   wrong.** `src/config.js:60` is authoritative and says `remedies: null`.
   `orch.example.yml:128` on origin/orch/integration ALSO already says
   `remedies: null  # ordered subset of rebase, rotate, reauthor, ask; default:
   fallback order` — it is correct and needs no fix. **Only the design doc's §15
   prints the literal `[rebase, rotate, reauthor, ask]` as the default, and only
   design §15 needs correcting separately** (not edited here, per the
   write-to-my-path-only instruction). I documented `null` as the default, said
   what it means (the failure table's own per-class order), and answered the
   follow-up an operator will actually hit — an explicit list that omits `ask`
   filters the slot out of every class's row, so a class whose only remaining
   option was `ask` goes straight to its terminal outcome and the run stops at
   cap with exit 2 instead of pausing to ask. Written up in §5.1's new
   `automation.remedies` note, with §4.3's shorter statement left in place.

5. **recon open decision 9 (which of `merge:`/`landing:` wins when both are set)
   is answered by the source.** `src/config.js:349-352`: within a source,
   `has(landingSource, "landing") ? "landing" : "merge"` — `landing` wins, and
   `merge:` still emits its rename warning. Documented as fact, no OPEN raised.

6. **The `--json` event set is smaller than the design describes.** recon's
   newModel prose lists nine event types (`run.start`, `cycle.start`, `failure`,
   `remedy`, `remote.readiness`, `human.ask`, `merge.request`, `merge.verified`,
   `run.end`). Grepping every `event: "..."` literal in src/ finds only six:
   `run.start`, `run.resume`, `run.detached`, `run.end`, `merge.request`,
   `merge.verified`. `cycle.start`, `failure`, `remedy`, `remote.readiness` and
   `human.ask` are emitted nowhere. Documented the six as current and marked the
   other five "not yet landed" in §4.6.

7. **`--max-attempts` does not exist as a flag.** recon's newModel and breaking
   change [29] both reference it (`--max-attempts 0` giving one pass). It is
   absent from `FLAGS` in src/schema.js, so it is a usage error today. I
   documented `automation.maxAttempts` (which does exist, config.js:59) and did
   not describe the flag.

8. **recon's phrasing "a reviewer crash records as ERROR rather than DISAGREE"
   is right but for a slightly different reason than "AGREE/DISAGREE is a token
   parsed off the end".** The crash path never reaches the parse decision: in
   src/adapters/cli-adapter.js:624-630, a nonzero run with a *line-anchored*
   DISAGREE keeps that verdict, and only a nonzero run with no usable verdict
   (or a prompt-echo, line 599) is flagged `agentError`. `agentError` is what
   produces `ERROR` in review-outcomes.jsonl (src/review-log.js). I wrote the
   precise version.

9. **`src/schema.js`'s comment on `COMMANDS.review` is WRONG, and it is the
   justification for a real behaviour.** It reads: *"review audits an existing
   branch's author (read off the branch name) and **never merges**, so nothing
   reads them"* — offered as the reason `--no-tidy`/`--author`/`--authors` are
   illegal on `review`. But `src/cli.js:2348` sets `noMerge: command === "pr"`,
   so a `review` run has `noMerge === false` and lands exactly like a `task`
   run; `engine.js:466`'s `if (noMerge)` early return is reached only by `pr`
   (and by `github.js:366`'s PR-bridge cycle, and by `buildAgent`'s
   `noMerge: !flags.pr`). The manual and Part 7 therefore say `orch review`
   merges, which matches the code. Two consequences worth a maintainer's
   attention, neither of which I acted on because I was told not to touch
   tracked files: (a) the schema comment should be corrected, since a future
   reader may "fix" the code to match the comment; and (b) `--no-tidy` being
   illegal on a command that *does* merge is now unjustified — a `review` run
   that lands leaves branches behind that `--no-tidy` cannot be asked to keep.
   This is exactly the kind of thing the repo's CLAUDE.md says to file an issue
   for; I have flagged it here instead of filing, per the write-to-my-path-only
   instruction.

10. **The streaming output format in §2.4 is taken from source, not from the old
   manual, which had none.** Everything is `notify.phase` (`src/notify.js:7-13`)
   writing `▸ <label>  <detail>` to **stderr**; the phase labels are
   `worktree`, `author`, `revise`, `review`, `gate`, `merge` (engine.js:109,
   180, 207, 220, 301, 338, 368, 420, 422, 493, 529, 544). Notably: the round
   line reads `(round N)` with **no `/roundCap`**; the security scan and the
   post-merge re-test print **nothing** on success; and the heartbeat is
   `… <label> still running (<elapsed> elapsed)` off-TTY versus a CR-rewritten
   `▸ <stage>  <label>   <word><dots>      <elapsed>` on a TTY
   (cli-adapter.js:160-163). The only `▶`-bulleted stdout line in a run is
   `▶ post-merge: docs-update spawned` (cli.js:121). The merged reason string is
   `agreed + green + integrated → PR <url>` (finalize.js:325-327).

11. **The cycle worktree is named after the branch, not the sid**:
   `join(orchDir, "wt", branch.replace(/\//g, "_"))` (cli.js:2294, 2315). Since
   the branch already ends in the sid, the two are easy to confuse; the manual
   says so explicitly.

12. **`--allow-large-scope` has exactly one effect**, which I verified by
   grepping every read: it flows from cli.js into engine.js:346, into
   cli-adapter.js:587 as the `{{allowLargeScope}}` template variable, and
   nowhere else. It does not touch `src/scope.js` (`scope.maxLines`) or
   `src/security-review.js`. The manual says so explicitly because the opposite
   is the natural guess.

OPEN callouts left in the document: **ZERO.** All six were resolved by owner
decision and rewritten as ordinary prose; the "Marks used" preamble no longer
documents an OPEN mark, because none exists to document. What each became:

- §3.4 (draft PR) → prose. NOT a forward-looking decision: already live on both
  refs. `src/readiness.js:63-64` folds `data.isDraft` into the same closed test
  as a closed/merged PR and returns the draft-specific summary `pr #N is a
  draft`; `REMOTE_PR_CLOSED`'s remedy row is `["ask"]` (`src/failure.js:133`),
  so a draft under `--until merged` asks a human and ends at exit 4 (no reply)
  or exit 3 (`orch: abandon`). One readiness predicate, exactly as decided. No
  "Not yet landed" marker — it would have been false.
- §3.5 (bare `continue` inherits the recorded goal) → prose, and the inheritance
  claim itself is UNMARKED because it is already live on both refs:
  `src/cli.js:2911` resolves the resume goal from `priorRun?.policy?.until`, and
  `src/cli.js:2995-2998` hands that recorded goal to the run controller. The
  section's "Not yet landed" marker is deliberately NARROW — it covers only the
  typed `--until ready|merged` override on the resume line, which `validate()` in
  `src/schema.js:292` still refuses. The marker names that guard as the one line
  that has to change. Note the ADJACENT claim in the same section — the fresh
  attempt budget on continue — IS live and is deliberately
  unmarked: `src/cli.js:2967` calls `runRecord.resumeTerminal(...)`.
- §3.6 (`agent add --build` keeps `noMerge`) → prose, unmarked for the noMerge
  half (it is current: `src/cli.js:1816`, comment at `:1768`) and marked only for
  the P12 command fold. The rationale is spelled out: the artifact is a module
  orch will import and execute to drive other agents, so a green gate proves the
  adapter's tests pass, not that it spawns the binary you think it does.
- §5.1 (`landing: pr` survives) → prose citing the validated enum
  `["ff-only", "no-ff", "pr"]` at `src/config.js:174`, with the cost of choosing
  the opt-out stated (no two-speed path, one PR per cycle, no autoBump).
- §5.2 (`rotateModels` + a pinned model) → prose. Already live on
  origin/orch/integration; see contradiction A above.
- §6.3 (`--allow-large-scope`) → prose. Survives unchanged and is ADVISORY: one
  string substitution into `{{allowLargeScope}}`, gating nothing mechanically.
  Command set written from the source rather than from the brief, which named
  only task/issue/agent build: the flag is in `RUN_FLAGS` (task, issue, review,
  continue), on `pr` (`src/schema.js:135`) and in
  `SUBCOMMAND_FLAGS["agent build"]`. One wrinkle named rather than papered:
  `SUBCOMMAND_FLAGS["agent add"]` is `["config-file", "dry", "build"]`, so after
  the P12 fold that line has to gain the flag or
  `agent add --build --allow-large-scope` will not parse.

Two questions the brief asked about were NOT open in this document and needed no
edit here — they live in a sibling draft. Answers from the source, for whoever
owns that file: `--json` is command-scoped, not global — `GLOBAL_FLAGS` is
exactly `["help", "version"]` (`src/schema.js:56`, whose comment says `--json` is
deliberately not global), and `json` is declared on `config`:91, `task`:102,
`issue`:109, `review`:117, `continue`:125, `pr`:135, `dashboard`:143. And `--dry`
is valid on `orch completion install` only: `completion` is
`mutates: true, flags: ["dry"]` (`src/schema.js:160`) and `validatePositionals`
at `:406` refuses `--dry` unless `rest[0] === "install"`. Both are now stated as
fact in §4.6 and §5.8 respectively.

The brief's decision 8 (migration guide recommends the incremental path) does
not apply to this file: the manual has no migration-guide section. Its
before/after pairs are per-feature and already incremental, and the two changes
that are safe to make TODAY are already called out as such (`merge:` → `landing:`
in §5.1, adding `--until once` to scripts in §1.6).

Deliberately NOT raised as OPEN, because the source answers them: recon open
decisions 4, 7, 8 (warn-window behaviour, documented inline in §5.1), 9, 10
(poller ownership — I note in §3.3/§5.7 that the poller lives outside this repo
only where relevant), 12 (harness/orch-loop.sh — documented in §5.7 as the
worked example needing an exit-code update).

Gaps from recon's docs.gaps closed here: (1) agent prompts §1.3 — both files
verbatim plus per-instruction consequences and the configurability answer;
(2) `.orch/` map §5.5; (3) run-output anatomy §2.4; (4) `--until`/`--detach`/
`--json`/`config --check` documented in §4.1/§4.6/§2.2; (5) status-header drift
is out of scope for the manual but the "Not yet landed" marks reflect real
state; (6) `harness/` §5.7; (7) first-ten-minutes Part 2 plus the agent-quota
caveat in §3.7; (8) no man page / bash-only completion §5.8.
-->
