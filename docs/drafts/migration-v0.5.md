...
Verified against the v0.4.360 checkout at
/mnt/nas-soul/nfs/stepa.local/projects/agent-orch (git clean, commit 733760b).
Where recon.json and the source disagreed, the source won. Discrepancies:

1. recon change 14 ("--until given twice with different values is an error")
   understates the landed behaviour. src/cli.js:496-502 refuses ANY repeated
   non-boolean flag, including two identical values. Probed:
   `orch task "x" --until ready --until ready` → "orch: --until given more than
   once", exit 64. recon's "for scalars the parser keeps last-one-wins" is not
   true of the current code, and recon's Before form does not run.

2. recon change 32 ("The README's 'MCP can never merge' promise is revoked")
   quotes README.md:169 as still containing "no way to emit `--merge`". It does
   not. That sentence has already been rewritten to the mcpMayMerge wording, and
   src/mcp.js:218 already enforces the opt-in. The row is landed, not pending.
   I documented it as already live and kept the original promise as the "Before"
   for historical clarity.

3. recon change 31 ("MCP `until` defaults to `ready`") cites mcp.js:72 as
   defaulting orch_task to "once". mcp.js:72 is `requireUntil`, which is
   consumed only by orch_pr (mcp.js:157). orch_task, orch_issue and
   orch_continue have NO `until` property today and their inputSchemas set
   `additionalProperties: false`, so passing one is currently an error. The
   break is "these tools gain a parameter that defaults to ready", not "the
   default flips". Documented that way.

4. recon change 2's rationale claims today's `runPr` "merges on orch's own
   AGREE+green and never calls prChecksGreen/statusCheckRollup". Stale:
   src/cli.js:2084 already maps `--merge` to `until: "merged"`, which routes
   through src/readiness.js (which requests
   mergeable,mergeStateStatus,statusCheckRollup at readiness.js:26). I dropped
   that clause and kept only the naming-collision argument plus the goal
   semantics.

5. recon's config.keys entry for `env.passthrough` says adapters "currently
   inherit the whole process.env". That contradicts recon's own change 34 and
   the source: src/adapters/cli-adapter.js:272 already builds the child env with
   `allowlistEnv(process.env)`. The allowlist IS live; `env.passthrough` is the
   inert part (validated at config.js:243-245, read nowhere).

6. recon change 23 ("unknown config keys are errors") is presented as a v0.5
   change. Unknown-key detection is already live in v0.4.360, on BOTH
   `config --check` (exit 1) and the run path (probed: `orch task "x" --dry`
   with `roudCap: 3` → "orch: orch.yml: unknown key 'roudCap'", exit 1). I split
   the row: unknown-key detection = landed; the REMOVED keys graduating from
   warning to error = P12.

7. recon change 25 (github.mergeMethod no longer applies to the standing PR) is
   already live, and confirmed on the REQUEST path, not just the event payload:
   src/landing.js:227 is
   `const method = landing === "pr" ? (cfg.github?.mergeMethod || "squash") : "merge";`
   so standing/base landings always send a merge commit. cfg.github.mergeMethod
   survives only on the `landing: pr` route (landing.js:227), foreign PRs
   (github.js:401) and the autoMergePr path (github.js:515).

7b. `--max-attempts` is documented in the v2 design but does NOT exist on this
   checkout: it is absent from src/schema.js FLAGS and from `orch --help`, and
   `orch task "x" --max-attempts 0` exits 64 with
   "orch: unknown option --max-attempts". The config key
   `automation.maxAttempts` IS live (cli.js:2099). Marked not-yet-landed in both
   places the flag is mentioned.

7c. Checklist step 4 ("delete the `main:` block on 0.4.x") is safe: config.js
   358-376 copies automation.conflictResolvers → cfg.main.conflictResolutionResolvers
   and automation.conflictAutoPaths → cfg.main.autoResolveConflictPaths whenever
   the canonical spelling is present in either layer, and the canonical spelling
   wins within a layer. Verified by reading, not probed end-to-end.

7d. NOT verified: whether `agent add --dry` actually acts on the flag. It is
   accepted (no exit 64), but `orch agent add foo --dry` on a scratch repo exits
   0 with "orch: unknown agent: foo" — an unregistered adapter name errors before
   any dry-run path is reached, so the probe proves acceptance only. The §3.5
   `--dry` row was softened accordingly.

8. recon changes 26/27 (exit-code split, exit 4): both LANDED in v0.4.360.
   src/run-controller.js:11 carries the full map
   {READY:0,MERGED:0,ERROR:1,STOPPED_AT_CAP:2,BLOCKED:3,WAIT_TIMEOUT:4}; the ask
   remedy exists at src/remedies/ask.js and consumes
   automation.humanWaitHours (ask.js:169) and returns exit 4 (ask.js:91);
   blockedReason is emitted on run.end (cli.js:2644, 3219). This RESOLVES recon
   openDecision 7 ("which release actually contains exit code 4") — P7 has
   landed ahead of P12. One caveat I documented: under `--until once`,
   cli.js:1726 and cli.js:2667 still raise a flat 2 for escalation /
   merge-deferred, so the split applies to ready/merged runs today.

9. recon's newModel prose lists nine --json event types (run.start, cycle.start,
   failure, remedy, remote.readiness, human.ask, merge.request, merge.verified,
   run.end). Only six exist in src: run.start, run.end, run.resume,
   run.detached, merge.request, merge.verified. cycle.start, failure, remedy,
   remote.readiness and human.ask are not emitted. Marked not-yet-landed.

10. Stale line citations in recon: cli.js:1507 is cited for author fan-out but
    is detach-log code; the fan-out is at cli.js:2261-2282. engine.js:252 is
    cited for reviewer unanimity; the actual verdict computation is
    engine.js:339-366.

11. recon openDecision 4 (automation.remedies default): RESOLVED by owner
    decision — `null` is correct (src/config.js:60), meaning "use the failure
    table order". The design doc §15 and orch.example.yml both print the literal
    `[rebase, rotate, reauthor, ask]` and are WRONG; they must be corrected
    separately (NOT edited by this pass — this document only notes it). §3.3 now
    documents `null`, says the two sources are wrong, and answers the follow-up
    about omitting `ask` from failure.js:180-195 + 204/228/239: an explicit list is a
    subtractive priority override, so omitting `ask` empties the allowed list for
    ask-only classes and `chooseRemedy` returns terminalDecision(row) =
    STOPPED_AT_CAP (exit 2, no comment posted) instead of exit 4.

12. recon openDecision 9 (merge: and landing: set together): resolved
    empirically for the warn window — `landing:` wins
    (src/config.js:349-355 picks "landing" when a source has both) and `merge:`
    still warns. Verified via `orch config --json` with `merge: pr` +
    `landing: ff-only` → landing = ff-only. Documented as observed v0.4.360
    behaviour, not as a v0.5 rule.

13. recon openDecision 6 (bare `orch continue` until default): RESOLVED by owner
    decision — a bare continue INHERITS the original run's recorded goal, and an
    explicit --until overrides. Verified landed on BOTH refs, so §1's marker was
    wrong to call the whole thing unlanded:
      main:        cli.js:2957 (controllerPolicy) / cli.js:2965 (resumePolicy)
      integration: cli.js:2995 / cli.js:3003
    `until: flags.until || priorRun.policy.until` — stored policy is the default,
    typed flag is the override. What is genuinely NOT landed is only the explicit
    override for the two non-`once` values: schema.js:292-293 rejects them on
    `continue` before cli.js is reached. §1's marker was rewritten to scope the
    not-yet-landed claim to that half only.

=== 2026-08-29 REVISION PASS: owner decisions applied ===

All eight owner decisions were handed down. Six touch this document:
  1. `landing: pr` survives  → §3.3, callout replaced with prose
  2. rotateModels composes with pinned models → §3.3 rewritten, §1 remedy 2
  3. remedies default is null → §3.3, callout replaced (see note 11 above)
  4. `agent add --build` keeps noMerge → §3.2, callout replaced with prose
  6. `continue` inherits the recorded goal → §1 (see note 13 above)
  8. incremental migration recommended → §4, callout replaced with prose
Two had no callout here and were folded into existing prose:
  5. `pr --until merged` refuses a draft → §2. Split into landed vs not:
     the PREDICATE already refuses (readiness.js:63-64, landing.js:136-137 both
     return REMOTE_PR_CLOSED "pr #N is a draft"), but the RESPONSE does not —
     failure.js:133 routes REMOTE_PR_CLOSED to the `ask` remedy, so today a
     draft target produces a question-and-wait ending in exit 4, not the direct
     refusal the decision describes. The refusal therefore carries a
     Not-yet-landed marker. For the P12 implementer: this is a one-row change in
     the failure table, not a readiness change.
  7. --allow-large-scope survives, advisory only → §6 bullet, sourced to
     schema.js:31/61/74, cli-adapter.js:587 and prompts/review.md:5,9-11. It is
     interpolated into the reviewer prompt and lifts ONE prompt instruction
     (reject a diff bundling >~3 logical changes). It gates nothing.
     CORRECTION to the decision text, which says "task, issue and agent build":
     the flag is live on SIX commands, not three. RUN_FLAGS (schema.js:61) is
     spread into task (:102), issue (:109), review (:117) and continue (:125) —
     it survives both filters — pr names it explicitly (:135), and agent build
     carries it at :74. It is read on those paths too, not merely accepted:
     cli.js:2865 (continue) and github.js:341,366 (pr). §6 says "every command
     that runs a review round" and enumerates, rather than repeating "three".

=== VERIFIED AGAINST origin/orch/integration (read-only; no checkout) ===

Method: `git show origin/orch/integration:src/<f>` and
`git diff main origin/orch/integration -- src/`. Integration is ahead of main by
7 commits touching src/: cli.js (+100/-53), config.js (+14/-8), remedies.js
(+133/-25). Nothing else in src/ differs; schema.js, mcp.js, failure.js,
readiness.js, landing.js, engine.js are BYTE-IDENTICAL on both refs, so every
schema/MCP/exit-code claim in this document holds on both.

ONE claim was contradicted by integration and has been corrected:
  - "automation.rotateModels ... remains inert until the rotate remedy consumes
    them" was true of the v0.4.360 tag and is FALSE on integration. #567 landed
    (merge 4850610 "Closes #567", with d8c0c7e "fix(rotate): use configured model
    ladders" and five follow-up fixes). config.js:61's defaults comment was
    rewritten in that change from "model ladders remain inert until the rotate
    remedy consumes them" to "optional per-agent model ladders consumed by the
    rotate remedy". remedies.js:175-188 (`nextModelRole`) is the consumer;
    config.js:227-236 tightened validation (duplicate-free + adapter must exist).

Every remaining `> **Not yet landed** (P12, #528).` marker was re-checked on
integration and is still accurate:
  banner            cli.js:1318 maybePrintRunBanner still present
  config wizard     cli.js:2022 runConfigWizard still called; readline at :825
  bare = once       cli.js:2121 `flags.until || (pr && merge ? "merged":"once")`
  orch review       schema.js COMMANDS.review still present (row at :117-119)
  orch update       schema.js:154 alias still present
  agent build       schema.js:73-74 both subcommands still present
  --pr              schema.js:74 still in "agent build" flags
  --max-attempts    absent from schema.js FLAGS on both refs
  --json events     only 6 emitted on both refs (run.start/end/resume/detached,
                    merge.request, merge.verified); cycle.start, failure, remedy,
                    remote.readiness, human.ask are still unemitted
  orch_review MCP   mcp.js identical to main
  MCP until param   mcp.js identical to main
  main: block       config.js:86 `main: new Set()` — children still unnamed
  authors/reviewers fan-out (#532) — integration's cli.js changes touch role
                    EXCLUSION matching (model-scoped exclusions), not the
                    fan-out; fan-out is unchanged on both refs

=== The two callouts the revision brief asked me to resolve from source ===

Neither existed in this document (they belong to the flag-reference draft):
  - `--json` global vs command-scoped: schema.js:56 is
    `export const GLOBAL_FLAGS = ["help", "version"];` — `--json` is NOT global.
    It is declared per command: config (:91), task (:102), issue (:109),
    review (:117), continue (:125), pr (:135), dashboard (:143). This document
    already stated it correctly by implication; no edit needed.
  - `--dry` on `completion install`: schema.js:155-163 marks `completion`
    `mutates: true, flags: ["dry"]`, and schema.js:401-406 rejects `--dry` on
    bare `orch completion` from validatePositionals with its own message. §3.5
    already documents both halves as fact; no edit needed.

=== OPEN callouts remaining in this document: ZERO ===

All four are resolved and the questions leave no trace. One IMPLEMENTATION
DEFECT is flagged in prose (§3.2, under the `--pr` row) — not an open decision:

  - Decisions 1 and 4 both hold, and they do not conflict in intent: a pull
    request IS the human checkpoint decision 4 asks for. What cannot express
    both is the current one-line spelling of `noMerge`. engine.js:466 returns
    `{status:"approved"}` BEFORE the finalize() call that reads cfg.landing, and
    cli.js:1779 (integration :1816) sets `noMerge: !flags.pr`. So `--pr` is
    currently the ONLY thing that makes a build reach finalize() at all. Delete
    `--pr` and leave `noMerge: !flags.pr` and the expression is unconditionally
    true — `landing: pr` becomes dead for a build and the row's "After" advice
    is a no-op. One line fixes it: cli.js:1779 → `noMerge: cfg.landing !== "pr"`,
    which keeps the build off the integration branch in every case while letting
    a `landing: pr` repo get its PR. Flagged for the P12 implementer.

Deliberately NOT flagged here, as they belong to other documents in the set:
poller ownership (recon #10).

Not verified end-to-end (documented from the design, marked not-yet-landed):
the exact wording of the post-cutover hard-error messages for removed keys;
the JSON-RPC -32601 text for orch_review; the `orch review x` exit-64 message.
-->
