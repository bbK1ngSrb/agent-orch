...
test("clean merge on a resumed cycle (task === branch): entry is never the branch slug", async () => {
  ...
  await finalize({ ...bumpCtx(), task: "pr/claude/x-1", closes: 53 }, deps);
  assert.equal(
    bumpArgs.entry,
    "release bookkeeping (no work-order text recorded) (closes [#53](https://github.com/bbk1ng/agent-orch/issues/53))",
  );
});
...
test("guard 2 gets cfg.gateTimeout as a wall-clock cap (#505)", async () => {
  let gateArgs = null;
  const { deps } = baseDeps({
    gate: { run: (...a) => { gateArgs = a; return { pass: true, log: "" }; } },
  });
  await finalize({ ...ctx(), cfg: { ...ctx().cfg, stageTimeout: 30, gateTimeout: 7 } }, deps);
  assert.equal(gateArgs[2], 420_000, "gateTimeout is threaded to the gate in ms");
