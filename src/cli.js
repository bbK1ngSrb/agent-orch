...
function persistRotationState({ orchDir, sid, runId, run, nextRun, previousAuthor }, stores = { inflight, runRecord, resume, checkpoint }) {
  ...
  if (run.mode === "task" && nextRun.authorName !== previousAuthor) {
...
export async function main(argv, deps = {}) {
          ...
          if (until !== "once") {
            ...
            const freshCycle = async (options = {}) => {
              ...
              if (picks) {
                ...
                const nextRun = { ...activeRun, ...rolePicks, resume: true };
  ...
  if (command === "continue") {
    ...
    const branchAuthor = branch.split("/")[1];
    const inflightHasReplacementAuthor = Boolean(inf?.author?.agent && inf.author.agent !== branchAuthor);
    if (((inf && !ck) || (ck?.stage === "started" && !ck.task))
      && !inflightHasReplacementAuthor
    ...
    const authorName = authorSpec.agent;
    const replacementAuthor = authorName !== branchAuthor;
    ...
    const run = {
      ...
      persistReviewers: replacementAuthor ? reviewers : (persistedReviewers || reviewers),
      ...
      reviewerOverride: reviewerOverride || excludedNames.size > 0,
      cfg, orchDir, repo, worktree: join(orchDir, "wt", branch.replace(/\//g, "_")),
    };
    ...
    if (!dry) {
      ...
      registerWithConcurrencyCap(
        orchDir,
        sid,
        { branch, pid: process.pid, baseSha, closes, author: authorSpec, reviewers: run.persistReviewers, workOrder, excludedAgents: run.excludedAgents },
        cfg,
        { onExceeded: (live) => { throw Object.assign(new Error(`orch: concurrency cap ${cfg.concurrency} reached — ${live} cycles live; try again shortly`), { exit: 3 }); } },
      );
    }
    ...
    const continueDeps = !dry && replacementAuthor && existsSync(`${run.worktree}.orch-preserve`)
