...
const AGENT_BUILD_ONLY_FLAGS = ["allow-large-scope", "author", "authors", "reviewer", "reviewers"];
...
export const HELP_PAGES = {
  ...
  agent: {
    ...
    about: [
      ...
      "A build never merges. An agreed and green adapter stays on its branch for a human to read and land, because code orch wrote that orch will then run as an agent gets a human checkpoint. Set landing: pr to open a pull request instead of leaving the branch bare.",
    ],
    ...
    flagOrder: ["build", "allow-large-scope", "author", "authors", "reviewer", "reviewers", "config-file", "dry"],
    flagHelp: {
...
export function validate(command, flags, { detachedChild = false } = {}) {
  ...
  if (!spec) {
    // No command at all (a bare command-specific flag) must not fall through
  ...
  if (flags.json && ["task", "issue", "pr"].includes(effective) && flags.until === "once") {
...
// `agent add --build` shares a positional shape (<name>) with the old build
// workflow. Build-only flags are meaningless on a plain registration.
function validateAgentArgs(rest, flags) {
  ...
  // Build-only options are meaningful only when --build is explicitly asked
  // for. A known adapter still follows the normal add path, so these options
  // remain rejected there rather than being silently ignored.
  let known = true;
  ...
  if (known || !flags.build) {
    for (const flagName of AGENT_BUILD_ONLY_FLAGS) {
      if (flags[flagName] !== undefined && flags[flagName] !== false) {
        throw usageError(`--${flagName} is not valid with 'orch agent add ${name}'${known ? " — the adapter is already registered" : " without --build — it only affects the build"}`);
      }
    }
  }
