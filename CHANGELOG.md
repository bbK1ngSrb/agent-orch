...
- feat (telemetry): `runs.jsonl` cycle lines now carry the run-level context a cycle cannot know on its own — `runId`, `until`, `attempt`, `detached`, and, for a cycle a remedy produced, `remedy`, `failureClass` and `humanWaitMs` (design §16). A `redrive` that fails quietly writes a `verdict: "merge-deferred", redrive: true` line instead of writing nothing at all. The run record additionally keeps the `readiness` observation and `mergedBy` that justify a green exit; the run-level terminal fields stay on the record rather than on `runs.jsonl`, because `notify.js` is protected and §16 defines the success metric over run records.
- feat (tooling): `scripts/v2-metrics.mjs` audits the programme's success metric over `.orch/run-records` — clean-unattended rate, false ready/merged, duplicate merge requests. A `--until merged` run counts as proven only if its record carries both a merge commit and the `verifiedAncestorAt` stamp that ancestry verification writes.
