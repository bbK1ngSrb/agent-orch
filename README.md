...
- **The merge is bound to the scanned commit, not a branch name.** A branch name is a
  moving pointer, so orch resolves it to a commit OID once — after AGREE and green tests,
  just before the deterministic security scan — and the scan, the changed-path floor and
  the merge all act on that same OID: what was scanned is what lands. Just before merging
  ...
  `branch head integrity check failed` instead of acting on a commit the scan never saw.
  Deferred cycles awaiting an automatic redrive carry that OID too, so the rebase is a
  compare-and-swap and a legacy record with no OID stays human-owned. The pin is not
  retroactive, and the manual (§1.1) is explicit about the two limits that follow: a push
  landing between the last review round and the pin inherits an AGREE it never earned, and
  a redrive's rebased commit is re-gated by the tests only — never re-scanned.
