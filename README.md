...
Run local coding agents (Claude, Codex, Copilot, Gemini, agy) in a cross-audit loop on any git repo.
...
- At least one agent CLI installed: `claude`, `codex`, `copilot`, `gemini`, `agy`, or
...
## Agents
`claude`, `codex`, `copilot`, `gemini`, `agy`, and three local-llm models served via llama-swap behind
...
> **External testers:** use the `claude` / `codex` / `copilot` / `gemini` / `agy` CLI agents. `llama-swap` is
